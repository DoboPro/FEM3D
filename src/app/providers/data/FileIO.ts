...
import { FemDataModel, FENode } from 'src/app/providers/data/FemDataModel';
import { BarParameter, Material } from './Material';
import { ShellParameter } from './ShellParameter';
...
export class FileIOService {
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      if (ss.length > 0) {
        ...
        // 梁パラメータ
          ...
          this.model.barParams.push(new BarParameter
  ...
  // 局所座標系を読み込む
  // ss - データ文字列配列
  public readCoordinates(ss: string[]): Coordinates {
    ...
    for (let i = 0; i < 3; i++) {
      const ci: number[] = c[i];
      let cf: number = ci[0] * ci[0] + ci[1] * ci[1] + ci[2] * ci[2];
      if (cf === 0) {
